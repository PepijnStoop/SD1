...
def addSource():
    ...
    for currentAddRow in range(1,maxRows+1):
        cellValue = sheetSource.cell(row=currentAddRow+1, column=3)
...
def removeInventory():
    ...
    sheetRemove.delete_rows(idx=sheetRemove.max_row+1, amount=10)
    removeRow = []

    for column in sheetRemove['C']:
        removeRow.append(column.value)

    for currentRemoveRow in range(1,maxRows):
        cellValue = sheetRemove.cell(row=currentRemoveRow, column=3)
        if cellValue.value not in existingRows:
            ...
            for currentRemoveRow in range(1,maxRows):
                cellValue = sheetMain.cell(row=currentRemoveRow, column=3)
...
def main():
    ...
    while option != 4:
        ...
        print("3. Delete from current inventory.")
        ...
        if option == 1:
            ...
            deleteEmpty()
#            printProducts()
            workbookMain.save('magazijn.xlsx')
